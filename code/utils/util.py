...
def save_mask_only(pred, save_path='./img/1/example_mask.png'):
    # 如果pred是二维的，扩展维度
    if len(pred.shape) == 2:
        pred = pred.unsqueeze(0)
    ...
    mask = np.zeros((h, w, 3))  # 初始化一个全黑的掩码

    # 遍历每个类别，将前景区域设置为白色
    for i in reversed(range(classes)):
        mask[pred[i] == 1] = (255, 255, 255)  # 前景为白色

    mask = mask.astype(np.uint8)  # 转换为整数类型，方便保存
