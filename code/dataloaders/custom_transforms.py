...
class RGBToHSL():  # 新定义的类
    def __call__(self, img):
        if not isinstance(img, Image.Image):
            raise TypeError("Expected img to be a PIL Image")
        ...
        # 转换回 PIL 图像
        return Image.fromarray(hsl_img)
class RGBToHSV():
    def __call__(self, img):
        # 检查输入是否为 PIL 图像
        if not isinstance(img, Image.Image):
...
class RGBToYCbCr():
    def __call__(self, img):
        ...
        Cr = 0.499 * r - 0.587 * g - 0.114 * b

        # 合并 YCbCr 通道
        ycbcr_img = np.stack((Y, Cb, Cr), axis=-1)

        # 将 YCbCr 的值映射到 [0, 255] 范围
        ycbcr_img = (ycbcr_img * 255).astype(np.uint8)
