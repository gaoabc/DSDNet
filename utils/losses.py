...
class HSIC_LOSS(nn.Module):
    ...
    # def forward(self, x, y, sigma=2):
    #     loss = self.hsic(x, y, sigma)
    #     return loss
    def forward(self, x, y, z, sigma=2, lam=3):
        loss = self.hsicloss(X=x, Y=y, Z=z, sigma=sigma, lam=lam)
        return loss
